...
public class GetEntityAPI {
	...
	public static Entity parse(String id, String jsonLine) {
	    ...
	    if(jobject.has("label")){
	    	...
	    	if(label.has("label_zh")){
	    		label_zh = label.get("label_zh").getAsString();
	    	}
	    	if(label.has("label_en")){
	    		label_en = label.get("label_en").getAsString();
	    	}
	    }
	    ...
	    if(jobject.has("superClasses")){
	    	for(Map.Entry<String, JsonElement> entry : jobject.getAsJsonObject("superClasses").entrySet()){
		    	String label_zh_2 = entry.getValue().getAsJsonObject().get("label_zh").getAsString();
		    	String label_en_2 = entry.getValue().getAsJsonObject().get("label_en").getAsString();
	    ...
	    for(Map.Entry<String, JsonElement> entry : jobject.getAsJsonObject("properties").entrySet()){
	    	...
	    	String label_zh_tmp = obj.getAsJsonObject("label").get("label_zh").getAsString();
	    	String label_en_tmp = obj.getAsJsonObject("label").get("label_en").getAsString();
	    ...
	    // new entity
		return new Entity(id, uri, label_zh, label_en, mention_list, abstract_en, abstract_zh, super_classes_en, super_classes_zh,
				related_items_zh, related_items_en, infobox_zh, infobox_en);
	}

	public static void main(String[] args) {
		System.out.println(GetEntityAPI.getEntityDetailByID("87320").toString());
	}
